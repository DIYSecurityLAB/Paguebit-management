...
export default function WithdrawalsStatusChart({ withdrawalsByStatus, loading, height = 250 }: Props) {
  ...
  const data = Object.entries(withdrawalsByStatus).map(([status, count]) => ({ status, count }));
    ...
    <ResponsiveContainer width="100%" height={height}>
      <BarChart data={data} layout="vertical">
        ...
        <YAxis 
          ...
          width={85}
        />
        <Tooltip contentStyle={{ background: '#18181b', color: '#fff', border: 'none' }} formatter={(value) => [`${value} saques`, 'Quantidade']} />
